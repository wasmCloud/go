...
do
  # Get the relative path of the file from the repo root
    ...
    target_dir="$docs_repo/docs/Examples/Go/${rel_path^}"
...
# Now check if there are any git changes in the docs repo
if [[ -z "$(git -C "$docs_repo" status --porcelain)" ]]; then
