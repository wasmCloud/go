...
type TraceState = []witTypes.Tuple2[string, string]
...
type SpanContext struct {
	...
	TraceState []witTypes.Tuple2[string, string]
}
...
func OnStart(context SpanContext) {
	...
	result3 := witRuntime.Allocate(pinner, uintptr(length*(4*4)), 4)
...
func OnEnd(span SpanData) {
	...
	returnArea := uintptr(witRuntime.Allocate(pinner, (40 + 32*4), 8))
	...
	*(*int32)(unsafe.Add(unsafe.Pointer(returnArea), (24 + 12*4))) = int32(((span).EndTime).Nanoseconds)
	...
	result9 := witRuntime.Allocate(pinner, uintptr(length10*(4*4)), 4)
	...
	*(*uint32)(unsafe.Add(unsafe.Pointer(returnArea), (32 + 12*4))) = uint32(uintptr(uintptr(result9)))
	...
	result18 := witRuntime.Allocate(pinner, uintptr(length19*(16+4*4)), 8)
	for index, element := range slice17 {
		...
		*(*int32)(unsafe.Add(unsafe.Pointer(base), (8 + 2*4))) = int32(((element).Time).Nanoseconds)
		...
		result15 := witRuntime.Allocate(pinner, uintptr(length16*(4*4)), 4)
	...
	*(*uint32)(unsafe.Add(unsafe.Pointer(returnArea), (32 + 14*4))) = uint32(uintptr(uintptr(result18)))
	...
	result34 := witRuntime.Allocate(pinner, uintptr(length35*(9*4)), 4)
	for index, element := range slice33 {
		...
		*(*int8)(unsafe.Add(unsafe.Pointer(base), (1 + 4*4))) = int8(result22)
		...
		result26 := witRuntime.Allocate(pinner, uintptr(length27*(4*4)), 4)
		...
		*(*uint32)(unsafe.Add(unsafe.Pointer(base), (5 * 4))) = uint32(uintptr(uintptr(result26)))
		...
		result31 := witRuntime.Allocate(pinner, uintptr(length32*(4*4)), 4)
	...
	*(*uint32)(unsafe.Add(unsafe.Pointer(returnArea), (32 + 21*4))) = uint32(uintptr(uintptr(utf837)))
	...
	result43 := witRuntime.Allocate(pinner, uintptr(length44*(4*4)), 4)
...
func OuterSpanContext() SpanContext {
	...
	result := make([]witTypes.Tuple2[string, string], 0, *(*uint32)(unsafe.Add(unsafe.Pointer(returnArea), (6 * 4))))
	...
	result3 := SpanContext{TraceId: value, SpanId: value0, TraceFlags: uint8(uint8(*(*uint32)(unsafe.Add(unsafe.Pointer(returnArea), (4 * 4))))), IsRemote: (uint8(*(*uint32)(unsafe.Add(unsafe.Pointer(returnArea), (1 + 4*4)))) != 0), TraceState: result}
	return result3
